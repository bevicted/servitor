...
Private-only and Satellite auth publication is deferred. Their owner-thread `auth` requests reply `VPN-backed authentication is not implemented yet` and do not queue delivery. Cleanup removes the publisher binding and Secret before waiting for in-flight work, then removes the publisher Role and ServiceAccount, preventing a late publisher from recreating data.
