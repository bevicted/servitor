...
FROM --platform=${BUILDPLATFORM} golang:${GO_VERSION}-bookworm AS servitor-build
ARG TARGETOS
ARG TARGETARCH
...
RUN CGO_ENABLED=0 GOOS=${TARGETOS} GOARCH=${TARGETARCH} go build -trimpath -ldflags='-s -w' -o /out/servitor-task ./cmd/servitor-task

FROM --platform=${BUILDPLATFORM} golang:1.23.0-bookworm AS ict-build
ARG TARGETOS
ARG TARGETARCH
WORKDIR /src/ict
...
RUN go mod download
COPY ict ./
RUN CGO_ENABLED=0 GOOS=${TARGETOS} GOARCH=${TARGETARCH} go build -trimpath -ldflags='-s -w' -o /out/ict .
